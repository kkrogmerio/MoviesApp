...
import { insertDataInDb,clearDb } from "../../database/realm/actions/manageMoviesData";
...
export const fetchMovieDataRemote = (chars) => {
  return async (dispatch) => {
    let fetchedData = await axios.get(
      `https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=${theMovieDbApiKey}&page=1`
    );
